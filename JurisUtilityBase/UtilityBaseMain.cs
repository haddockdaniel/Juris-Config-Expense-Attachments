...
//            listBoxCompanies.SelectedIndexChanged += listBoxCompanies_SelectedIndexChanged;
        ...
        private int getCodeFromSelection()
        {
            if (radioButtonCode0.Checked)
                return 0;
            if (radioButtonCode1.Checked)
                return 1;
            if (radioButtonCode2.Checked)
                return 2;
            return 4; //means they didnt pick a radiobutton
        }


        private bool isMoneyAmount()
        {
            try
            {
                double.Parse(textBoxAmount.Text);
                Regex reg = new Regex(@"[0-9]{1,3}(?:,?[0-9]{3})*\.[0-9]{2}$");
                Match match = reg.Match(textBoxAmount.Text);
                if (match.Success)
                    return true;
                else
                    return false;
